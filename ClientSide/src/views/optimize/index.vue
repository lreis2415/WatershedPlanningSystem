    ...
    <Content>
      <div class="bmp-content">
        <Row :gutter="16" style="margin-bottom:20px;">
          <Col span="12">
          <Card :bordered="true">
            <p slot="title"><b>{{ $t("message.targetScenarioSpatial") }}</b></p>
            <div id="map" ref="rootmap" style="height: 465px"></div>
            <img class="legend" src="@/assets/img/legend.jpg" />
          </Card>
          </Col>
          <Col span="12">
          <div>
            <Card :bordered="true">
              <p slot="title"><b>{{ $t("message.targetScenarioInfo") }}</b></p>
            ...
            <Button type="success" @click="goHistoryPage()" style="margin-top:20px">
        ...
        <Form class="mb20" :model="form" ref="form" :rules="rules" :label-width="150">
          ...
          <Row :gutter="16">
            <Col span="12">
            <Card :bordered="true">
              <p slot="title"><b><span style="color:#ed4014;">Step 1: </span>{{ $t("message.setNameAndNotes") }}</b></p>
              <FormItem :label="$t('message.optName')" prop="name">
                <Input type="text" v-model="form.name" :placeholder="$t('message.plhdOptName')" style="width: 400px"
                  @on-blur="checkName">
                </Input>
              </FormItem>
              <FormItem :label="$t('message.optNotes')" prop="notes">
                <Input type="textarea" v-model="form.notes" :placeholder="$t('message.plhdOptNotes')"
                  style="width: 400px">
                </Input>
              </FormItem>
            </Card>
            <Card :bordered="true" style="margin-top:20px">
              <p slot="title"><b><span style="color:#ed4014;">Step 2:</span> {{ $t("message.setStepwiseInvest") }}</b>
              </p>
              <Tabs :animated="false">
                <p class="tip">{{ $t("message.lblSetInvestCaps") }}
                  <InputNumber :max="300" :min="200" v-model="totalInvestmentCap" />
                  <span style="margin-left:10px;color:#ed4014;">{{ investDiffText }}: {{ investDiffDisplay }} </span>
                </p>
                <TabPane :label="$t('message.tabMultistageInvestCap')">
                  <Row :gutter="16">
                    <Col span="12">
                    <div>
                      <FormItem class="fc-row" :label="$t('message.firstYear')" prop="invesments" :label-width="100">
                        <Slider v-model="form.invesments[0]" @on-change="handleSliderChange" show-input></Slider>
                      </FormItem>

                      <FormItem class="fc-row" :label="$t('message.secondYear')" prop="invesments" :label-width="100">
                        <Slider v-model="form.invesments[1]" @on-change="handleSliderChange" show-input></Slider>
                      </FormItem>

                      <FormItem class="fc-row" :label="$t('message.thirdYear')" prop="invesments" :label-width="100">
                        <Slider v-model="form.invesments[2]" @on-change="handleSliderChange" show-input></Slider>
                      </FormItem>

                      <FormItem class="fc-row" :label="$t('message.fourthYear')" prop="invesments" :label-width="100">
                        <Slider v-model="form.invesments[3]" @on-change="handleSliderChange" show-input></Slider>
                      </FormItem>

                      <FormItem class="fc-row" :label="$t('message.fifthYear')" prop="invesments" :label-width="100">
                        <Slider v-model="form.invesments[4]" @on-change="handleSliderChange" show-input></Slider>
                      </FormItem>
                      <FormItem class="fc-row" :label="$t('message.discountRate')">
                        <InputNumber :max="20" :min="0" v-model="form.discountRate"></InputNumber>
                      </FormItem>
                    </div>
                    </Col>
                    <Col span="12">
            ...
            <Col span="12">
            <Card>
              <p slot="title"><b><span style="color:#ed4014;">Step 3: </span>{{ $t("message.optAlgorithmParameters")
}}</b></p>
              <Collapse v-model="value1">
                <Panel name="1">
                  ...
                  <template #content>
                    <List border>
                      <ListItem><b>{{ $t("message.maxReductionRate") }}</b></ListItem>
                      <ListItem><b>{{ $t("message.minNPV") }}</b></ListItem>
                    </List>
                  </template>
                </Panel>
                <Panel name="2">
                ...
                <Panel name="3">
                  ...
                  <template #content>
                    <FormItem :label="$t('message.genNumbers')">
                      <InputNumber :max="100" :min="2" v-model="form.generation"></InputNumber>
                      <Button type="primary" @click="resetParams()">{{ $t("message.reset") }}</Button>
                    </FormItem>

                    <FormItem :label="$t('message.popSize')">
                      <InputNumber :max="200" :min="4" v-model="form.population"></InputNumber>
                    </FormItem>
                    <FormItem :label="$t('message.crossoverRate')">
                      <InputNumber :max="1" :min="0" :step="0.1" v-model="form.crossoverRate"></InputNumber>
                    </FormItem>
                    <FormItem :label="$t('message.mutateRate')">
                      <InputNumber :max="1" :min="0" :step="0.1" v-model="form.mutateRate"></InputNumber>
                    </FormItem>
                    <FormItem :label="$t('message.selectRate')">
                      <InputNumber :max="1" :min="0" :step="0.1" v-model="form.selectRate"></InputNumber>
                    </FormItem>
                  </template>
                </Panel>
              </Collapse>
            ...
            <Card style="margin-top:20px">
              <p slot="title"><b>{{ $t("message.submitOpt") }}</b></p>
              <Row>
                <Col span="7">
                <Button type="primary" :loading="loadingOnline" @click="handleSubmit('form', 'online')"
              ...
              <Divider />
              <Row>
                <Col span="7"><Button type="primary" :loading="loadingOffline" @click="handleSubmit('form', 'offline')"
                  style="margin-top: 10px;">
                  {{ $t("message.submitOptOffline") }}
                </Button>
                </Col>
                <Col span="17">{{ $t("message.optOfflineTip") }}
                </Col>
              </Row>
            </Card>
...
export default {
  ...
  methods: {
    ...
    getSelectedScenario() {
      axios
        .all([
          axios.get('/api/unit_to_gene.json'),
        ...
        .then(resArr => {
          ...
          this.scenarioData = [
            ...
            },
            {
              ...
              money2: this.selectedScenario.BMPS.SHENGTAILINCAO.INCOME.toFixed(
                2
              )
            },
            {
              ...
              money: this.selectedScenario.BMPS.DIXIAOLINGAIZAO.CAPEX.toFixed(
                2
              ),
              money1: this.selectedScenario.BMPS.DIXIAOLINGAIZAO.OPEX.toFixed(
                2
              ),
              money2: this.selectedScenario.BMPS.DIXIAOLINGAIZAO.INCOME.toFixed(
                2
              )
            },
            {
    ...
    handleSubmit(name, mode) {
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          let url = mode == 'online' ? '/api/submitBMPOrdersOptimizationOnline' : '/api/submitBMPOrdersOptimizationOffline'
          axios
            .post(url, root.form, {
              headers: headers
            })
    ...
    goHistoryPage() {
      ...
      axios(`/api/hasHistoricalOptimizations?groupId=${groupId}`).then(res => {
        if (res.data && res.data.code == 1) {
  ...
  computed: {
    ...
    barOption() {
      return {
        xAxis: {
          type: 'category',
          data: [this.$t('message.firstYear'), this.$t('message.secondYear'), this.$t('message.thirdYear'), this.$t('message.fourthYear'), this.$t('message.fifthYear')],
          axisLabel: {
            fontSize: 14,
